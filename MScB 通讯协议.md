...
| 指令ID |      指令参数       |           指令含义           |
| :----: | :-----------------: | :--------------------------: |
|  0x00  | DISABLE_CONTROLLING | 停止MScB所有对舵轮的信息传输 |
|  0x01  | ENABLE_CONTROLLING  | 开始MScB所有对舵轮的信息传输 |
...
GET_PID_PARAMETER (0x01) 作用：获取某个舵轮的指定 PID 参数

SET_PID_PARAMETER (0x02) 作用：设置某个舵轮的指定 PID 参数

##### 主机发送数据

|         数据区段          |            内容            | 数据类型  |
| :-----------------------: | :------------------------: | :-------: |
|          cmd_id           |        0x01 / 0x02         | uint8_t:5 |
...
> 💡	也就是说，MScB响应数据和主机发送的 29Bit 拓展帧相同，仅有 8 Byte 数据帧是不同的
...
|  数据区 1 区段  |             内容             | 数据类型 |
| :-------------: | :--------------------------: | :------: |
| Byte 3 ~ Byte 2 | 比例环节二次方输出放大幂次数 | uint16_t |
...
| Byte 3 ~ Byte 2 | 积分环节二次方输出放大幂次数 | int16_t  |
...
|  数据区 1 区段  |             内容             | 数据类型 |
| :-------------: | :--------------------------: | :------: |
| Byte 3 ~ Byte 2 | 微分环节二次方输出放大幂次数 | uint16_t |
